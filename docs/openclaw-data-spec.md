# MangoGo × @BB + Story 数据采集与文案规范 v1.1

> 本文档约束两个 Agent 的协作流程：
> - **@BB**：负责景点素材采集（图片、原始文字、基础信息）
...
> Story 使用 Qwen 大模型，负责将 @BB 采集的原始素材加工为真实自然的旅行文案。
...
**Step 1：接收素材（等待 @BB 完成原始采集）**
- 确认收到 @BB 输出的 `meta.json`，检查 `geo`、`info`、`images` 字段是否完整
...
- 不得删除 @BB 已填写的地理类标签（城市名、景点名）

**Step 5：补充 tips（可选）**
- 若 @BB 提供的 tips 过于笼统（如"建议早去"），Story 可基于景点性质补充更具体的建议
...
│               负责人审查原始素材                        │
│  · 检查图片命名、字段完整性、坐标准确性                   │
│  · approved → 交给 Story；rejected → 打回 @BB    │
└──────────────────────┬──────────────────────────────┘
                       │ 确认后交棒
                       ▼
┌─────────────────────────────────────────────────────┐
