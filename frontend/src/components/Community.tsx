...
export default function Community({ isLoggedIn }: { isLoggedIn: boolean }) {
  ...
  return (
    <div className="flex-1 bg-[#F8F9FA] px-4 py-8 min-h-screen max-w-7xl mx-auto w-full">
      ...
      {showUpload && (
        <>
          <div className="fixed inset-0 bg-transparent z-[9998]" onClick={() => setShowUpload(false)}></div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {uploadedImages.map((img, index) => (
                      <div key={index} className="relative group aspect-square w-[120px] h-[120px]">
                        ...
                        <div className="absolute inset-0 bg-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex items-center justify-center gap-2">
                          {index > 0 && (
      ...
      {showLoginPrompt && (
        <>
          <div className="fixed inset-0 bg-transparent z-[10000]" onClick={() => setShowLoginPrompt(false)}></div>
